module ApplicationHelper
  def user_status
    ...
    if logged_in?
      ...
      links[2] = link_to current_user.username.to_s, user_path(current_user.id), class: 'nav-link'
    else
    ...
    if logged_in?
      if events_attended.find_by(id: event.id)
        ...
        results << "<button class='btn btn-primary'>
         #{link_to 'Attend',
                   attendances_path(controller: 'attendances',
                                    action: 'create',
                                    event_id: event.id), method: :post,
                                                         class: 'text-decoration-none text-light'} </button> "
